...
def visualize_individual_metrics(rfm_df):
    ...
    axs[0].annotate(f'{rfm_df["Recency"].mean():.2f}', 
...
def visualize_rfm(rfm_df):
    ...
    rfm_df[['Recency', 'Frequency', 'Monetary']].mean().plot(kind='bar', color=['orange', 'blue', 'green'])
...
def visualize_high_risk_distribution(rfm_df):
    ...
    plt.pie(high_risk_counts, labels=['Low Risk', 'High Risk'], autopct='%1.1f%%', startangle=90, colors=['lightblue', 'salmon'])
